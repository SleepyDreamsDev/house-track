...
- **Dashboard staleness banner**: Above the KPI strip, driven by `/sweeps/latest` (`finishedAt ?? startedAt`). Error banner when the latest sweep `status === 'failed'`; warning banner when the latest sweep is not running and ended more than `STALE_SWEEP_THRESHOLD_MS` (24h, hardcoded client const — cadence is a few sweeps/day, so 24h means at least two missed sweeps) ago. Both link to `/sweeps`. No banner while a sweep is running that started within the threshold (a long-overdue sweep that just started shouldn't warn); a `running` sweep whose `startedAt` is older than the threshold is a zombie (process died mid-sweep, row never finalized) and DOES warn. No banner when no sweep exists yet (fresh DB / query error — first-boot stays clean).
