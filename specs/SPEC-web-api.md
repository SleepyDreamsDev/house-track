...
**GET /api/listings/:id/dossier:** `{ activeDOM, domMedianDistrict (null when district unknown), hedonic: { predictedEur, residualPct } | null, postedAt, bumpedAt, authorName, authorListings: [{ id, url, title, priceEur, active, delistedAt }] (same authorId, excl. self; [] when authorId null), cluster: { canonicalId, members: [{ id, url, title, priceEur, active, delistedAt, firstSeenAt }] } | null }`. Hedonic fit per-request over the `active && !excluded` slice (same idiom/floor as `/analytics/valuation`); district DOM median reuses `districtDomMedians()` from analytics.ts computed over the price+area-complete subset of the slice — the same population `/analytics/motivated-sellers` uses, so the two views report the same median. Cluster = `canonicalId ?? id` resolved over ALL listings (no active filter — delisted siblings are the relist story), self excluded, `null` when no siblings. 404 `'Listing not found'` on unknown id. Lives in `src/web/routes/listings.ts` (NOT mcp/queries.ts) to keep the spec surface web-api-only.
